...
namespace QueryParser
{
    ...
    {
        internal static string ToProperty<T, TOut>(this Expression<Func<T, TOut>> propertyExpression)
        {
            var visitor = new MemberAccessPathVisitor();
            visitor.Visit(propertyExpression);

            var attrs = visitor.Property.GetCustomAttributes(true);

            if (attrs.Where(x => x is ParserIgnoreAttribute).Count() >= 1)
                return string.Empty;
            if (attrs.Where(x => x is ParserPropertyName).Count() >= 1)
                return (attrs.Single(x => x is ParserPropertyName) as ParserPropertyName).PropertyName.ToLower();

            return visitor.Property.Name.ToLower();
        }
    }


    internal class MemberAccessPathVisitor : ExpressionVisitor
    {
        ...
        protected override Expression VisitMember(MemberExpression methodExp)
        {
            ...
            Property = methodExp.Member as PropertyInfo;
